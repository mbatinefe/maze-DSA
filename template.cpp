...
bool CheckUVneighbor(Cell<int>** maze, int M, int N, Cell<int> c){
    ...
    cout << "Searching for unvisited neighbors of maze[" << c.x << ", " << c.y << "]";
    int count = 0;
        ...
        if (askedStack != nullptr) {
            ...
            */
            ...
            while(CheckUVneighbor(maze[k], M, N, askedStack->top())){
                ...
                if(temp_rest_next == temp_rest_base){
                ...
                askedStack->push(maze[k][temp_rest_base.row][temp_rest_base.col]);
                askedStack->push(maze[k][temp_rest_next.row][temp_rest_next.col]);

            }

            while(!askedStack->isEmpty() && visit_count > 0){
                Cell<int> temp_rest_next;
                Cell<int> temp_rest_base = askedStack->topAndPop();

                if(CheckUVneighbor(maze[k], M, N, temp_rest_base)){
                    // If there is no unvisited neighbors, pop the last element
                    // and continue to search for unvisited neighbors
                                    // Returned updated next cell
                    temp_rest_next = SelectNeighbor(maze[k], M, N, maze[k][temp_rest_base.row][temp_rest_base.col]);
                    // cout << "Selected Neighbor Rest: " << temp_rest.x << " " << temp_rest.y << endl;
                    if(temp_rest_next == temp_rest_base){
                    // If there is no unvisited neighbors, pop the last element
                    // and continue to search for unvisited neighbors
                    ...
                    cout << "Visited Count Recursive: " << visit_count << endl;
                    visit_count--;
                    askedStack->push(maze[k][temp_rest_next.row][temp_rest_next.col]);
